...
class ImageDetailRepository {
  ...
  /// 投稿を削除する
  Future<void> deleteImage(ImageEntity imageEntity, RoomInfoEntity roomInfoEntity) async {
    
    // FireStoreから投稿を削除
    await Firestore.instance.document('rooms/${roomInfoEntity.roomId}/images/${imageEntity.image_id}').delete();
    
    // Cloud Storageから画像を削除
    await FirebaseStorage.instance.ref().child('roomImages/${roomInfoEntity.roomId}/${imageEntity.created_at}').delete();

    // Cloud Storageからサムネイル画像を削除
    await FirebaseStorage.instance.ref().child('roomImages/${roomInfoEntity.roomId}/thumbnails/${imageEntity.created_at}_200x200').delete();
