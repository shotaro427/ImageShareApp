...
class AddMemberRepository {
  ...
  void inviteUser(String email, RoomInfoEntity roomInfoEntity) async {
    ...
    if (await _validateSameUser(_user, roomInfoEntity)) throw Exception('This user is already invited');

    // ユーザーのwaitingRoomsに追加
    final _addRoom =  _userRef.collection('waitingRooms').add({
      'room': _roomRef
    });
  ...
  Future<bool> _validateSameUser(DocumentSnapshot user, RoomInfoEntity roomInfoEntity) async {
    final String _targetUid = user.data['uid'];

    final QuerySnapshot _pendings = await Firestore.instance.document('rooms/${roomInfoEntity.roomId}').collection('pending').getDocuments();
    final QuerySnapshot _members = await Firestore.instance.document('rooms/${roomInfoEntity.roomId}').collection('participants').getDocuments();
