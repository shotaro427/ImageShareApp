...
void main() {
  group("ImageUploadBlocのテスト", () {
    test('pickUpImage()のテスト', () async {
      ...
      when(_repository.getImageInGallery()).thenAnswer(
          (_) => Future.value(File('test_resources/contacts.json')));

      target.pickUpImage();

      target.value.listen(expectAsync1((actual) async {
    ...
    test('uploadImage()のテスト', () async {
      ...
      when(_repository.uploadImageToFireStorage(testFile, 'roomId', any))
          .thenAnswer((_) => Future.value(null));
      when(_repository.postImageWithTitle('roomId', any))
          .thenAnswer((_) => Future.value(null));
